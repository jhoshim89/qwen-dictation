...
> **Measurement update (applied):** After the research below, we measured Qwen3-ASR inference on this Mac (mps): **~0.1 s per pass for 1–8 s windows**. That overturns the research's "0.8 s keeps a slow local model from backlogging" assumption — the model is not the bottleneck; the **poll interval** is. So the shipped values are tighter than the generic research defaults: **`STREAM_INTERVAL = 0.4`, `PAUSE_SILENCE_SEC = 0.4`, `MAX_WINDOW_SEC = 12.0`**. The table below records the research baseline; the measured machine speed is why `STREAM_INTERVAL`/`PAUSE_SILENCE_SEC` ship at 0.4 instead of 0.8/0.6.
