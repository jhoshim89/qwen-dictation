...
단, 받아쓰기 중 떠 있는 네이티브 HUD는 글쓰기 표면 위에 올라가므로 그림자와
내부 하이라이트가 없는 flat 2D 캡슐로 유지한다.
...
- **HUD bg (`#5A5658`):** 네이티브 recording HUD의 flat 2D 캡슐 배경. 글쓰기 입력창 위에서는 62% opacity로 뒤 텍스트가 읽히게 한다.
...
Pretendard Variable을 로컬 번들해 설정창에 사용한다. 네이티브 HUD는 현재
텍스트 상태 pill을 의도적 예외로 허용하되, 외부 CDN에 의존하지 않는다.
...
젤리 효과는 앱 아이콘, 주요 버튼, 상태 pill, 핵심 카드 헤더에만 사용한다.
네이티브 HUD는 예외적으로 solid fill과 solid 코랄 막대만 사용한다.
설정창 canvas에는 매우 약한 warm radial wash를 허용한다.
...
- **Recording HUD:** flat 2D 반투명 회색 캡슐 안에서 중앙이 가장 높은 코랄 막대 3개가 음량에 따라 부드럽게 움직인다. 막대 끝은 앱 아이콘처럼 `radius = width / 2`인 둥근 캡슐로 그린다. 기본 위치는 화면 하단에서 24px 위로, 입력 텍스트 줄을 가리지 않게 둔다. 그림자, 테두리, halo, 내부 하이라이트는 사용하지 않는다. pill 모드는 “듣는 중” 같은 짧은 상태 텍스트를 함께 보여도 된다.
